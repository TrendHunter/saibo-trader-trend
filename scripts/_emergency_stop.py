#!/usr/bin/env python3
"""Emergency: pause live trading + switch .env to shadow (no build)."""
...
def main() -> int:
    ...
    # 1. Pause immediately
    pause = json.dumps({"control": "pause", "reason": "user emergency stop — shadow only"})
    print(run(f"printf '%s' '{pause}' > '{PROJ}/logs/runtime_config.json'"))

    # 2. Shadow mode in .env (no real orders on next restart)
    for key, val in [
    ...
    print("=== env ===")
    print(run(f"grep -E '^LIVE_LIH_DRY_RUN=|^RISK_MAX_CONCURRENT' '{PROJ}/.env'"))
    ...
    print(run(
        ...
        "print(\"liveLihDryRun\", l.get(\"liveLihDryRun\")); print(\"openCount\", l.get(\"openCount\"))'"
    ))
    ...
    c.close()
    print("\n已停：实盘暂停，.env 已改 shadow。未编译、未 resume。")
    return 0
